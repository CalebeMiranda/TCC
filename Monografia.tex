...
tanto a justificativa textual quanto o resultado concreto da aplicação dos passes, replicando a estrutura esperada pelo
...
\cite{faustino2021new}, denominada de \textit{Optimization Cache} em conjunto com a comparação contra flags tradicionais do LLVM 
...
flags padrão encontra-se documentada na planilha \textit{Comparação Otimização Faustino vs Padrão} disponível no link: \href{https://github.com/Alisson-Teles/Pipeline-training/blob/main/AVR/Compara%C3%A7%C3%A3o%20Otimiza%C3%A7%C3%A3o%20Faustino%20vs%20Padr%C3%A3o%20-%20P%C3%A1gina1.pdf
}{do repositório}.

Dessa forma, os resultados práticos deste trabalho incluem os scripts: 
\begin{itemize}
    \item O script \textit{rodar_seq_uniq_AVR.sh} responsável por aplicar, para cada programa em LLVM-IR, todas as 1289 sequências de otimização únicas derivadas do trabalho de \cite{faustino2021new}. Compilando o código para AVR para cada sequência, armazenando os arquivos resultantes (.s, .o, .ll) e registrando, para cada programa, aquela sequência que produz o menor tamanho de objeto, definindo assim o “melhor caso”.
    \item O script \textit{parallel-run.py} que atua paralelizando a execução do \textit{rodar_seq_uniq_AVR.sh} em múltiplos núcleos da CPU, permitindo processar simultaneamente diversos códigos. Reduzindo drasticamente o tempo total de execução.
    \item O script \textit{coleta_melhores.py} que percorre todas as subpastas de resultados produzidas pelas compilações e extrai, para cada programa, o arquivo objeto melhor.o e seu tamanho em bytes. Esses valores são consolidados em um arquivo único \textit{melhores.csv}, organizado como uma tabela contendo o nome do programa e o tamanho pós otimização.
...
série de mudanças no planejamento inicial, que influenciaram diretamente o escopo deste trabalho. Diversas mudanças foram aplicadas no decorrer desta pesquisa,
...
Desta forma, a contribuição principal deste trabalho é o desenvolviemnto prático e teórico de um pipeline capaz de fornecer toda a base necessária para o refinamento futuro do modelo.

Outros problemas relevantes que causaram uma mudança no escopo deste projeto foi o custo computacional associado à execução do pipeline completo,
em especial a etapa de autotuning. A execução do método completo, de elaborado por \cite{faustino2021new} demandaria tempo e adicionaria complexidade 
a um projeto já limitado por prazos e recursos. Em função dessas limitações, optou-se por restringir o autotuning a uma lista predefinida de sequências 
a \textit{Optimization Cache} derivada de \cite{faustino2021new} e por processar um subconjunto controlado de programas priorizando a reprodutibilidade do pipeline, (da geração à produção 
...
razão da GPU utilizada (Nvidia A10G alugada via Hugging Face), o parâmetro \texttt{model\_max\_length} foi ajustado para 8192 tokens. 
...
Outros problemas técnicos superados, foi a conversão e mapeamento manual dos passes da lista \textit{Optimization Cache}, 
