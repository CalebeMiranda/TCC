...
O artigo de \cite{cummins2023large} introduz o LLM-Compiler, a primeira aplicação extensiva de Large Language Models ao 
...
Para este trabalho, \cite{cummins2023large} fornecem o fundamento conceitual e base prática (o próprio LLM-Compiler). Os autores provaram que LLMs podem aprender 
...
reduzido, necessidade de converter e mapear passes entre versões do LLVM e limites de tempo, por isso, a maior contribuição da pesquisa de \cite{cummins2023large} 
...
autotuning descrito por \cite{faustino2021new} e utilizado na lista \textit{Optimization Cache}. Assim, sua
...
conjunto inicial de candidatas, seguido da aplicação do algoritmo de redução de \cite{PuriniJain2013} para eliminar passes redundantes.
...
Para o presente TCC, a influência do trabalho de \cite{faustino2021new} é direta e prática: adoptamos a \textit{Optimization Cache} como fonte principal
...
ao fato de que alguns códigos não geraram sequências válidas ou apresentaram falhas durante a compilação e portanto são descartados do conjunto
...
\textbf{1\,721 pares} \textit{prompt}/\textit{label} destinado ao treinamento supervisionado do LLM-Compiler especializado 
...
flags padrão encontra-se documentada na planilha \textit{Comparação Otimização Faustino vs Padrão} disponível no link: \url{https://github.com/Alisson-Teles/Pipeline-training/blob/main/AVR/Compara%C3%A7%C3%A3o%20Otimiza%C3%A7%C3%A3o%20Faustino%20vs%20Padr%C3%A3o%20-%20P%C3%A1gina1.pdf
}.

Dessa forma, os resultados práticos deste trabalho incluem os scripts: 
\begin{itemize}
    \item O script \textit{rodar\_seq\_uniq\_AVR.sh} responsável por aplicar, para cada programa em LLVM-IR, todas as 1289 sequências de otimização únicas derivadas do trabalho de \cite{faustino2021new}. Compilando o código para AVR para cada sequência, armazenando os arquivos resultantes (.s, .o, .ll) e registrando, para cada programa, aquela sequência que produz o menor tamanho de objeto, definindo assim o “melhor caso”.
    \item O script \textit{parallel-run.py} que atua paralelizando a execução do \textit{rodar\_seq\_uniq\_AVR.sh} em múltiplos núcleos da CPU, permitindo processar simultaneamente diversos códigos. Reduzindo drasticamente o tempo total de execução.
...
proposto por \cite{faustino2021new}. A execução do método completo, demandaria tempo e adicionaria complexidade a um projeto já limitado por prazos e recursos. Em função dessas limitações, optou-se por restringir o autotuning a uma lista predefinida de sequências 
a \textit{Optimization Cache} derivada de outros trabalhos de \cite{faustino2021new} e por processar um subconjunto controlado de programas priorizando a reprodutibilidade do pipeline, em detrimento da exploração exaustiva do espaço combinatório de passes. 
...
razão da GPU utilizada (Nvidia A10G alugada via Hugging Face, que possui 12vCPU, 46 GB de RAM e 24 GB de VRAM), o parâmetro \texttt{model\_max\_length} foi ajustado para 8192 tokens. 
...
Apesar das limitações enfrentadas, o pipeline desenvolvido e o conjunto de dados produzidos configuram uma base sólida e valiosa para 
